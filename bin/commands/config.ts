...
export async function configure(
  ...
  try {
    const isNewConfiguration: boolean = await new Promise(async (resolve, reject): Promise<any> => {
      if (exists === true) {
...
[${args.get("environment") !== undefined ? args.get("environment") : "development"}]
themeid = ${args.get("themeid")}
storefront = "${args.get("storefront")}"
password = "${args.get("password")}"
${args.get("ignores") !== undefined ? `ignores = "${args.get("ignores")}"` : ``}
${args.get("directory") !== undefined ? `directory = "${args.get("directory")}"` : ``}
...
`;
    ...
    stderr(`${args.get("environment") === undefined ? "development" : args.get("environment")} environment already exists, please supply the --environment or -e flag to mango config`);
  }
}
